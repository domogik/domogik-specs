...
{CODE()}from domogik_packages.xpl.lib.lightplugin import LightingExtension
{CODE}

Add 3 callbacks fuction to your plugin :
{CODE()}    def lighting_activate_device(self, device, channel, level, faderate):
        '''
...
{CODE()}self._lighting = LightingExtension(self)
...
The full schema is available [http://xplproject.org.uk/wiki/index.php?title=Schema_-_LIGHTING|here]
If you want to participate, please do it.

!!!Developpers notes
!!!Notes about the lighting schema notion :
[http://xplproject.org.uk/wiki/index.php?title=Schema_-_LIGHTING#xPL_gateway|Gateway] : implemented
[http://xplproject.org.uk/wiki/index.php?title=Schema_-_LIGHTING#Network|Networks] : not implemented. Nobody use it for now, so we always work in the preferred network.  We silently ignore this option as mention in documentation.
[http://xplproject.org.uk/wiki/index.php?title=Schema_-_LIGHTING#Device|Devices] : implemented.
[http://xplproject.org.uk/wiki/index.php?title=Schema_-_LIGHTING#Channels|Channels] : implementation in progress. For now, one channel by device is developed but multichannels devices will be soon.
[http://xplproject.org.uk/wiki/index.php?title=Schema_-_LIGHTING#Fade_Rate|Faderate] : not implemented. Don't know if it will be. Options are present in messages but silently ignored.
[http://xplproject.org.uk/wiki/index.php?title=Schema_-_LIGHTING#Scene|Scenes] : Implemented.
...
[http://xplproject.org.uk/wiki/index.php?title=Schema_-_LIGHTING#Channels|Channels in lighting] are used to manage differents lights on the same device. 
